...
public class Suma extends JDialog implements ActionListener {
	...
	public Suma(VentanaPrincipal principal) {
		...
		comboMatriz1.setToolTipText("Seleccione el nombre de la matriz que desea sumar.");
		...
		comboMatriz2.setToolTipText("Seleccione el nombre de la matriz que desea sumar.");
		...
		botonAceptar.setToolTipText("Seleccione aceptar si desea sumar las matrices");
	...
	// Metodo que consiste en validar los escenarios posibles antes de sumar las matrices
	// seleccionadas.
	private void metodoAceptar() {
		...
		if (principal.arregloMatriz[comboMatriz1.getSelectedIndex()] == null
			...
			Matriz temp = null;
			temp = Matriz.suma(principal.arregloMatriz[comboMatriz1.getSelectedIndex()],
			...
			if (temp == null) {
				JOptionPane.showMessageDialog(this,
						...
						"Error al sumar", JOptionPane.ERROR_MESSAGE);
			} else {
				this.dispose();
