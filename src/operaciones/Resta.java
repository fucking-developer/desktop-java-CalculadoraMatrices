...
public class Resta extends JDialog implements ActionListener {
	...
	public Resta(VentanaPrincipal principal) {
		...
		comboMatriz1.setToolTipText("Seleccione el nombre de la matriz que desea restar.");
		...
		comboMatriz2.setToolTipText("Seleccione el nombre de la matriz que desea restar.");
		...
		botonAceptar.setToolTipText("Seleccione aceptar si desea restar las matrices");
	...
	// Método que consiste en validar los escenarios posibles antes de realizar la
	...
	private void metodoAceptar() {
		...
		if (principal.arregloMatriz[comboMatriz1.getSelectedIndex()] == null
			...
			Matriz temp = null;
			temp = Matriz.resta(principal.arregloMatriz[comboMatriz1.getSelectedIndex()],
					principal.arregloMatriz[comboMatriz2.getSelectedIndex()]);
			if (temp == null) {
				JOptionPane.showMessageDialog(this,
						"Ocurrió un error al realizar la resta \n " + "usar solo matrices de tamaño similar",
						"Error al restar", JOptionPane.ERROR_MESSAGE);
			} else {
				this.dispose();
